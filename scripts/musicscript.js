// player.js — assumes `songs` array and optional `pageArtwork` img element exist in scope
...
function syncMediaSession() {
    ...
    const artwork = typeof pageArtwork !== 'undefined' ? pageArtwork : null;
    navigator.mediaSession.metadata = new MediaMetadata({
        title:   songs[currentIndex].name,
        artist:  songs[currentIndex].artist,
        album:   document.title,
        artwork: artwork ? [{ src: artwork.src, sizes: '512x512', type: 'image/jpeg' }] : []
    });
...
setSong(0, false); // load first track without autoplaying